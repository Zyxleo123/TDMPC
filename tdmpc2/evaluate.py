...
def evaluate(cfg: dict):
    ...
    if cfg.save_dataset:
        dataset_dir = os.path.join(cfg.work_dir, "datasets")
    ...
    for task_idx, task in enumerate(tasks):
        ...
        for i in range(cfg.eval_episodes):
            ...
            if cfg.save_video:
                frames = [env.render()]
            while not done:
                action = agent.act(obs, t0=t == 0, task=task_idx, eval_mode=True, use_pi=cfg.eval_pi) # TODO: set use pi=True to evaluate nominal policy
                if cfg.save_dataset:
            ...
            if cfg.save_video:
                ...
                print(f"video {i} saved: {os.path.join(video_dir, f'{task}-{i}.mp4')}") # TODO
            print(f"eval episode {i}, ep_reward: {ep_reward}, ep_success: {info['success']}") # TODO
        ...
        if cfg.save_dataset:
            states_array = np.array(obs_list)   
            ...
            np.savez(os.path.join(dataset_dir, f"{task}.npz"), states=states_array, actions=actions_array)
            print(f"dataset saved to {os.path.join(dataset_dir, f'{task}.npz')}")
