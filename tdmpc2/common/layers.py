...
from tdmpc2.common.vectornet import VectorNetEncoder
...
class PixelPreprocess(nn.Module):
    ...
    def forward(self, x):
        return x.div_(255.0).sub_(0.5) # normailze to [-0.5, 0.5]
...
def enc(cfg, out={}):
    ...
    for k in cfg.obs_shape.keys():
        if k == "state":
            ...
            n_objects, n_steps, n_features = cfg.obs_shape[k]
            encoder = VectorNetEncoder(
                n_objects=n_objects,
                n_steps=n_steps,
                n_features=n_features,
                latent_dim=cfg.latent_dim,
                task_dim=cfg.task_dim,
                hidden_dim=cfg.get('vtn_hidden_dim', 64),
            )
            
            # Load pre-trained VTN weights if specified
            if cfg.get('vtn_pretrained_path', None):
                from tdmpc2.common.vtn_loader import load_pretrained_vtn_weights
                print(f"Loading pre-trained VTN weights from: {cfg.vtn_pretrained_path}")
                encoder = load_pretrained_vtn_weights(
                    encoder,
                    cfg.vtn_pretrained_path,
