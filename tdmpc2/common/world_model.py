...
class WorldModel(nn.Module):
    ...
    def encode(self, obs, task):
        ...
        """
        obs_key = self.cfg.obs
        
        # For waypointvec, don't add task embedding before encoding
        # VectorNetEncoder handles task_dim internally
        if obs_key == "waypointvec":
            return self._encoder[obs_key](obs)
        
        # For other observation types, add task embedding if multitask
        if self.cfg.multitask:
