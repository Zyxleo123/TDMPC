"""
TDMPC2 world model with insample value.
Under this case we do not require an explicit policy network
"""
...
class WorldModel(nn.Module):
    ...
    def V(self, z, task, return_logits=False):
        """
        Predict value.
        ...
        Args:
            ...
            return_logits: if True, return raw logits for soft_ce loss computation;
                          if False, return scalar values (default)
        """
        if self.cfg.multitask:
            z = self.task_emb(z, task)
        V = self._V(z)
        if return_logits:
            return V
        return math.two_hot_inv(V, self.cfg)
